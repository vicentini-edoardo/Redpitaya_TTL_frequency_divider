/*
 * rp_harmonic_ctl.c — Red Pitaya NCO harmonic generator register control helper
 *
 ...
 *   0x04  reserved
 *   0x08  mult_n            harmonic multiplier (3-bit, [1..5])
 *   0x0C  reserved
...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "      mult_n: integer 1..5 (harmonic order, clamped by FPGA)\n"
...
/* Write a 48-bit signed value into two 32-bit registers.
   Write hi first so the FPGA latch (triggered on lo write) sees a consistent value. */
...
static void print_json(volatile uint8_t *base) {
    ...
    const uint32_t mult_n         = rd32(base, REG_MULT_N) & 0x7u;
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[2], "read") == 0) {
        ...
        /* Clamp mult_n to [1, 5] in software as a sanity check */
        if (mult_n < 1u) mult_n = 1u;
        if (mult_n > 5u) mult_n = 5u;

        wr32(base, REG_CONTROL, 0);
        wr32(base, REG_MULT_N, mult_n);
        ...
        /* Bit 1 is self-clearing in the FPGA. Preserve bit 0. */
        const uint32_t ctrl = rd32(base, REG_CONTROL) & ~0x2u;
        wr32(base, REG_CONTROL, ctrl | 0x2u);
        print_json(base);

    } else if (strcmp(argv[2], "window") == 0) {
        /* Set measurement window duration in microseconds (min 1000 = 1 ms) */
        if (argc != 4) {
            usage(argv[0]);
            munmap(map, (size_t)page_size);
            close(fd);
