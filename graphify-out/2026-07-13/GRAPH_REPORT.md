...
## Corpus Check
- 16 files · ~21,281 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 292 nodes · 509 edges · 25 communities (12 shown, 13 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 10 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5ff93eeb`
...
## Knowledge Gaps
- **58 isolated node(s):** `Goal`, `Scope`, `Error Handling`, `Testing`, `Out Of Scope` (+53 more)
...
- **Why does `SshBackend` connect `SSH Backend Job Queue` to `Dark Workbench Style Tokens`, `hz_to_phase`, `rp_math.py`?**
  _High betweenness centrality (0.149) - this node is a cross-community bridge._
- **Why does `_NcoPanel` connect `Apply/Oscillator Math` to `Dark Workbench Style Tokens`, `PulsePanel`?**
...
- **What connects `Single persistent paramiko SSH session shared by both panels.      self._mode ("`, `Pulse-mode modulated write (enable=1, harmonic_mode=0 via helper).`, `Harmonic-mode modulated write (enable=1, harmonic_mode=1 via helper).` to the rest of the system?**
