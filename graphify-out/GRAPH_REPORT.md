...
## Corpus Check
- 13 files · ~20,223 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 279 nodes · 520 edges · 24 communities (12 shown, 12 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 10 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f2d8d181`
...
6. `hz_to_phase()` - 13 edges
7. `phase_to_hz()` - 13 edges
8. `Red Pitaya TTL Frequency Generator` - 13 edges
9. `Dark Workbench GUI Redesign` - 12 edges
10. `_mono_font()` - 11 edges
...
Cohesion: 0.09
Nodes (9): QObject, _Job, Pulse-mode modulated write (enable=1, harmonic_mode=0 via helper)., Harmonic-mode modulated write (enable=1, harmonic_mode=1 via helper)., Set control register via the pulse helper (keeps harmonic_mode=0)., Set control register via the harmonic helper (keeps harmonic_mode=1)., Write DIO2 48-bit NCO step. phase_step=0 disables., Single persistent paramiko SSH session shared by both panels.      self._mode (" (+1 more)

### Community 3 - "Dark Workbench Style Tokens"
Cohesion: 0.14
Nodes (17): QFont, QFrame, QGridLayout, QGroupBox, QLabel, QWidget, BigDisplay, _btn_style() (+9 more)

### Community 4 - "redpitaya_picosdk_verify.py"
Cohesion: 0.12
Nodes (9): Connection Panel (Host/Port/User/Key, Connect, Upload & Compile, status LED), Controls Panel (Freq shift, Width, Meas. window, Enable Output, Auto-Apply, Apply Now, Soft Reset), Frequency Shift Status Line (requested/actual/register/resolution readout), Log Panel (scrolling status/debug output), Red Pitaya TTL Frequency Divider GUI Screenshot (Pulse Tab, Disconnected), PulsePanel, Pulse / frequency-shift mode (harmonic_mode=0): f_out = f_in + f_shift., duty_to_cycles() (+1 more)
...
Cohesion: 0.08
Nodes (7): HarmonicPanel, _mode_btn_style(), _NcoPanel, Public entry point (e.g. the Ctrl+Return shortcut) → mode-specific write., Harmonic generator mode (harmonic_mode=1): f_out = N × f_in + f_shift., Style for the three output-mode buttons (OFF / MODULATED / ON)., Shared UI and logic for the two NCO control tabs.      Both tabs poll the same s
...
Cohesion: 0.10
Nodes (15): Measurement Stat Tiles (Input Frequency, Pulse Duration, Output Frequency, Duty Cycle), fmt_dur(), fmt_freq(), fmt_signed_freq(), hz_to_phase(), measured_edges_to_phase_step(), phase_to_hz(), suggest_window() (+7 more)

### Community 37 - "rp_math.py"
Cohesion: 0.11
Nodes (7): QMainWindow, main(), MainWindow, Switch the poll helper when the user changes tabs., Track the active measurement window (clamped) as the poll period., Switch poll target between pulse and harmonic helper., PySide6-Essentials>=6.6

## Knowledge Gaps
- **53 isolated node(s):** `axi4lite_pulse_regs`, `pulse_gen`, `system_wrapper`, `pulse_gen`, `axi4lite_pulse_regs` (+48 more)
...
- **Why does `SshBackend` connect `SSH Backend Job Queue` to `Dark Workbench Style Tokens`, `redpitaya_picosdk_verify.py`, `rp_math.py`?**
  _High betweenness centrality (0.156) - this node is a cross-community bridge._
- **Why does `_NcoPanel` connect `Apply/Oscillator Math` to `Dark Workbench Style Tokens`, `redpitaya_picosdk_verify.py`, `rp_math.py`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Why does `MainWindow` connect `rp_math.py` to `Apply/Oscillator Math`, `SSH Backend Job Queue`, `Dark Workbench Style Tokens`, `hz_to_phase`?**
